...
fn main() {
    ...
    let file = std::fs::File::open("./audios/beep.wav").unwrap();
    ...
    let file = std::fs::File::open("./audios/beep2.wav").unwrap();
    ...
    let file = std::fs::File::open("./audios/beep3.ogg").unwrap();
    let beep3 = stream_handle.play_once(BufReader::new(file)).unwrap();
